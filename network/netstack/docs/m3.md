...
栈地址与 `ip addr` 可相同（如 `10.0.0.1`）；关键是路由使目的为该地址的 TCP 段由内核写入 TUN。
...
cmake --build build
./build/examples/tun_tcp_echo tun0 10.0.0.1 8080
...
- [ ] `OpenTun` 在 Linux 上打开指定设备（非阻塞）
- [ ] `FdEndpoint::WritePacket` 将 IPv4 帧写入 TUN
- [ ] `FdEndpoint::PollOnce` 读入帧并交付 `DeliverNetworkPacket`
- [ ] `examples/tun_tcp_echo` 编译（`NETSTACK_ENABLE_TUN=ON`）
- [ ] 宿主机 `nc` 完成 TCP echo（手动）
- [ ] channel/ctest **11 项仍全部通过**（TUN OFF）
- [ ] **不要求**：TAP、ARP、路由表、waiter、IPv6、默认 ctest TUN
...
| 双配地址混淆 | 文档约定：`AddAddress` vs `ip addr` 分工写清 |
