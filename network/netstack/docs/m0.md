...
本文档是 C++ 重写 **M0** 阶段的实施指南。
架构背景见 [`refer-arch.md`](refer-arch.md)。
...
| 范围 | 链路环回 + IPv4 解析/校验 |
| 验收 | 单元测试喂原始帧（无 L2 头） |
| 下一里程碑 | M1：UDP echo + `channel` link（见 [`docs/m1.md`](m1.md)） |
...
- [x] `header` 单测通过：含 `IsValid` 边界与 checksum（`ctest -R header_ipv4`）
- [x] `loopback` / `channel`：`WritePacket` / `InjectInbound` → IPv4 → transport stub
- [x] 注入 **合法** 裸 IPv4 → payload 长度 = `TotalLength - IHL`（`tests/m0/ipv4_acceptance_test.cc`）
- [x] 注入 **非法** 帧（坏 checksum、短包、未知协议号）→ 丢弃 + 统计递增
- [x] `Stack::GetStats` 聚合 NIC 与 IPv4 计数
