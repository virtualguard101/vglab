...
enum class TcpState {
  ...
  kCloseWait,
  kLastAck,
  kTimeWait,   // M2 可立即转 Closed，不等 2MSL
};
```
...
- [ ] `seqnum` 单测通过（`ctest -R seqnum`）
- [ ] `header/tcp` 单测通过（`ctest -R header_tcp`）
- [ ] `Stack` 注册 TCP，`DeliverTransportPacket` 能到达 `tcp::Endpoint`
- [ ] channel 上完成 **三次握手**（SYN / SYN-ACK / ACK 可观测）
- [ ] **单段数据** 双向或单向交付正确
- [ ] **FIN 关闭** 后状态回到 `CLOSED`，端口可重新 `Listen`
