...
int run_command(int argc, char** argv, const ParsedArgs& args) {
  ...
  if (cmd == "scan" || cmd == "peers") {
    ...
    for (const auto& p : peers) {
      std::cout << p.address << ':' << p.tcp_port << "  id=" << p.node_id
                << "  host=" << p.hostname << '\n';
    }
    return 0;
  }
  ...
  if (cmd == "list") {
    ...
    const auto port = parse_port(
        flag_value(argc, argv, "--port", std::to_string(cfg.tcp_port)),
        cfg.tcp_port);
  ...
  if (cmd == "download") {
    ...
    const bool ok = transfer::download_file(
        io, host, port, remote, local, [](const DownloadProgress& p) {
          if (p.total == 0) return;
          const int pct = static_cast<int>((100.0 * p.received) / p.total);
          std::cout << "\r " << pct << "%  " << p.received << '/' << p.total
                    << std::flush;
        });
    std::cout << '\n';
    if (!ok) {
    ...
    if (!ec && elapsed > 0) {
      ...
      std::cout << "Saved to " << local << " (" << bytes << " bytes, " << mib_s
                << " MiB/s)\n";
    } else {
