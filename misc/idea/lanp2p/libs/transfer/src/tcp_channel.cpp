...
bool TcpChannel::read_message(std::vector<uint8_t>& out) {
  ...
  if (mh.length > 0 && !read_all(out.data() + protocol::kHeaderSize, mh.length))
    return false;
  return true;
}
