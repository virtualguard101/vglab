...
Server::Server(boost::asio::io_context& io, uint16_t tcp_port,
...
void Server::handle_session(tcp::socket socket) {
  ...
  if (type == MsgType::RequestFile) {
    ...
    if (!path) {
      channel.send_message(MsgType::Error, 0,
                           protocol::encode_string_body("file not found"));
      return;
    }
    ...
    if (ec) {
      channel.send_message(MsgType::Error, 0,
                           protocol::encode_string_body("stat failed"));
      return;
    }
    ...
    if (!in) {
      channel.send_message(MsgType::Error, 0,
                           protocol::encode_string_body("open failed"));
      return;
    }
    ...
    while (in) {
      ...
      if (got == 0) break;
      std::vector<uint8_t> body(chunk.begin(),
