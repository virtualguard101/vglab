...
FileIndex::FileIndex(std::filesystem::path root) : root_(std::move(root)) { refresh(); }
...
std::optional<std::filesystem::path> FileIndex::resolve(const std::string& relative_path) const {
  ...
  if (ec) return std::nullopt;
  auto mismatch = std::mismatch(root_canon.begin(), root_canon.end(), candidate.begin());
  if (mismatch.first != root_canon.end()) return std::nullopt;
  if (!std::filesystem::is_regular_file(candidate, ec) || ec) return std::nullopt;
  return candidate;
}
