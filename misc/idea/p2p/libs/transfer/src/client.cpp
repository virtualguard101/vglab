...
bool download_file(boost::asio::io_context& io,
  ...
  TcpChannel channel{std::move(socket)};
  if (!channel.send_string_message(MsgType::RequestFile, remote_path)) return false;
  ...
  while (true) {
    ...
    out.write(reinterpret_cast<const char*>(payload), static_cast<std::streamsize>(plen));
    if (!out) return false;
    received += plen;
    if (on_progress) {
