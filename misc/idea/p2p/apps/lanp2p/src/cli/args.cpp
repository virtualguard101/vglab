...
std::string arg_value(int argc, char** argv, const std::string& flag, const std::string& def = {}) {
...
void print_usage(const char* program) {
  std::cerr
      << "Usage:\n"
      << "  " << program << " run [--share-dir DIR] [--download-dir DIR] [--tcp-port N] [--udp-port N]\n"
      << "  " << program << " scan [--wait SECONDS]\n"
      << "  " << program << " peers\n"
      << "  " << program << " list --host HOST [--port N]\n"
      << "  " << program << " download --host HOST --remote PATH [--local PATH] [--port N]\n"
...
bool parse(int argc, char** argv, ParsedArgs& out) {
  ...
  out.config.download_dir = arg_value(argc, argv, "--download-dir", "./downloads");
  out.config.tcp_port =
      parse_port(arg_value(argc, argv, "--tcp-port", std::to_string(kDefaultTcpPort)), kDefaultTcpPort);
  out.config.udp_port =
      parse_port(arg_value(argc, argv, "--udp-port", std::to_string(kDefaultUdpPort)), kDefaultUdpPort);
